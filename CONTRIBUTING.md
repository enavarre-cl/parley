...
> menus, views), the extension **host** code (`src/host/**`), or the **webview** module graph
> (`src/webview/{app,core,render,ui,features,chat,panels}/**`) — anything that needs a rebuild. CSS
> (in `media/`) is served live, so a plain reload suffices for those.

## Source layout

All hand-written source lives under `src/`, split by the runtime it targets:

- `src/host/**` — the extension host (Node.js: `vscode`, `fs`, network, processes). Entry `extension.ts`.
...
The webview is **TypeScript** (`src/webview/**`), bundled by esbuild before it can run in the browser
...
- `scripts/build-webview.ts` (run via `tsx`, wired into `npm run dev` and `vscode:prepublish`) emits
  ...
  panel (`i18n`, `spell`, `models`, `modelsFormat`, `voices`, `compare`, `dictionary`, `engines`), and
...
npm run build:webview        # webview: esbuild src/webview → media/dist/*.js (valid bundle)
...
> **Typing convention:** both `src/host/` (host) and `src/webview/` (webview) are TypeScript and `any`-free.
...
> ambient globals in `src/webview/globals.d.ts`) — not `any`. Outbound objects the code *builds* must be
> fully typed.

## Adding a backend (provider)

Every backend implements one interface (`src/host/providers/types.ts`) and is wired through a small
factory. To add a provider `foo`:

1. **`src/host/providers/foo.ts`** — implement `LLMProvider`:
   ```ts
...
2. **`src/host/providers/index.ts`** — register it:
...
3. **`src/host/extension.ts`** — if it needs an API key, add `{ id: 'foo', label: 'Foo' }` to
...
6. **(Optional)** gate provider-specific sampling params via the `only:` arrays in the settings
   schema in `src/webview/panels/config.ts`.
...
> add its bundle, extend the `jotflow.language` enum, and `SPELL_LANGS` in `src/host/spellWords.ts`.
