(function () {
  ...
  // ---- Message rendering ----
  // Markdown rendering moved to markdown.js (loaded before this script); alias entry points.
  const escapeHtml = window.PMd.escapeHtml;
  const renderMarkdown = window.PMd.render;
  const renderMarkdownImpl = window.PMd.renderRaw;

  // Mermaid rendering moved to mermaid-view.js (loaded before this script).
  const processMermaid = (root) => window.PMermaid.process(root);
  ...
  // Find & replace lives in find.js (loaded before this script); bridge the entry points main.js uses.
  window.PFind.setApi(vscode);
  const openFind = window.PFind.open;
  const closeFind = window.PFind.close;
  const refreshFind = window.PFind.refresh;
  const setReplaceVisible = window.PFind.setReplaceVisible;
